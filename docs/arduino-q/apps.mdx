...
- [ ] In `main.py`, add a `print("Hello from Python!")` statement and run the App: check the output in the App Lab console.
- [ ] In `sketch.ino`, use `Monitor.println("Hello from Arduino!");` (after `Monitor.begin()` in `setup()`) and verify it appears in the App Lab **Monitor**, not the classic Arduino IDE Serial Monitor.
